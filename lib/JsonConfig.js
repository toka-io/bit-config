...
class JsonConfig {
  constructor(configDir) {
    let files = fs.readdirSync(configDir.trim());
    this.configurations = [];
  ...
  get(property) {
    ...
    let val = property.split('.').reduce((o, i) => (o === null) ? this.configurations[0][i] : o[i], null);
    if (val)
      return val;
    else
      throw new Error('Property not found. Please make sure the property exists in your config file.');
  }
}
